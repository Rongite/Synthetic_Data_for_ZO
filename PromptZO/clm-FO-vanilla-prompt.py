...
def write_results(args, losses, ppl):
    ...
    tag += f"-ntokens-{args.n_tokens}-seed-{args.seed}-lr-{args.learning_rate}"
    
    torch.save(losses, f'result/adam/{tag}-loss.pt')
    torch.save(ppl, f'result/adam/{tag}-ppl.pt')
            ...
            if (step > 0 and step % args.gradient_accumulation_steps) == 0 or step == len(train_dataloader) - 1:
                ...
                if completed_steps > 0 and completed_steps % 100 == 0:
                    ...
                    torch.save(model.soft_prompt.weight,
                                f'model/{optimizer_name}/{args.n_tokens}-step{completed_steps}_model_{args.block_size}-{args.learning_rate}-{args.seed}.pt')
