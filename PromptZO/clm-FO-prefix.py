                ...
                if completed_steps > 0 and completed_steps % 100 == 0:
                    ...
                    if args.lora_rank != -1:
                        torch.save({n : p for n, p in model.named_parameters() if p.requires_grad},
                                f'model/lora-ZO/{optimizer_name}-{args.dataset_name}-{args.dataset_config_name}-step{completed_steps}_model_{args.block_size}-{args.lora_basic_rank}-{args.lora_rank}-{args.learning_rate}-{args.ZO_eps}-{args.seed}.pt')
                    else:
                        torch.save({n: p for n, p in model.named_parameters() if p.requires_grad},
                                   f'model/lora-ZO/{optimizer_name}-{args.dataset_name}-{args.dataset_config_name}-step{completed_steps}_model_{args.block_size}-{args.lora_basic_rank}-{args.learning_rate}-{args.ZO_eps}-{args.seed}.pt')
