...
def write_results(args, losses, ppl):
    ...
    tag += f"-lr-{args.learning_rate}-eps-{args.ZO_eps}-{args.seed}"
    
    torch.save(losses, f'result/zo-vanilla/{tag}-loss.pt')
    torch.save(ppl, f'result/zo-vanilla/{tag}-ppl.pt')
        ...
        while completed_steps < args.max_train_steps:
            for step, batch in enumerate(train_dataloader):
                ...
                for _ in range(zo_sample):
                    ...
                    model = perturb_parameters_MeZO(model, args.ZO_eps, scaling_factor=1, seed=seed)                    
                    loss1 = model(**batch).loss

                    model = perturb_parameters_MeZO(model, args.ZO_eps, scaling_factor=-2, seed=seed)                    
                    ...
                    for n, p in model.named_parameters():
                        ...
                        if p.grad is None:
                            p.grad = torch.normal(mean=0, std=1, size=p.size(), device=p.device, dtype=p.dtype).mul_(projected_grad/args.zo_sample)
                        else:
                            p.grad.add_(torch.normal(mean=0, std=1, size=p.size(), device=p.device, dtype=p.dtype), alpha=projected_grad/args.zo_sample)
                ...
                if completed_steps >= args.max_train_steps:
                    ...
                    #             f'model/ZO/{optimizer_name}-{args.dataset_name}-{args.dataset_config_name}-model_{args.block_size}-{args.learning_rate}-{args.ZO_eps}-{args.seed}.pt')
                    break
