...
def write_results(args, losses, ppl):
    ...
    tag += f"-ntokens-{args.n_tokens}-{args.lora_rank}-seed-{args.seed}-lr-{args.learning_rate}"
    
    torch.save(losses, f'result/adam-svd-more-rank/{tag}-loss.pt')
    torch.save(ppl, f'result/adam-svd-more-rank/{tag}-ppl.pt')
...
def main():
    ...
    while completed_steps < args.max_train_steps:
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            loss = model(batch, soft_prompt=soft_prompt).loss
            ...
            if (step > 0 and step % args.gradient_accumulation_steps) == 0 or step == len(train_dataloader) - 1:
                ...
                if completed_steps > 0 and completed_steps % 100 == 0:
                    ...
                    torch.save((model.prompt_U_weight, model.prompt_V_weight),
                               f'model/{optimizer_name}-svd-more-rank/{args.dataset_name}-{args.dataset_config_name}-{args.n_tokens}-step{completed_steps}_model_{args.block_size}-{args.lora_rank}-{args.learning_rate}-{args.seed}.pt')
