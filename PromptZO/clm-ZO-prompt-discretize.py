...
def write_results(args, losses, ppl):
    tag = args.dataset_name + '-' + args.dataset_config_name + '-' + \
        ...
        str(args.block_size) + "-" + str(args.per_device_train_batch_size) + \
            "-" + str(args.zo_sample) + "-" 

    tag += "momentum"
    tag += f"-{args.n_tokens}-seed-{args.seed}-lr-{args.learning_rate}-eps-{args.ZO_eps}"

    # torch.save(losses, f'result/prompt-ZO-discretized/{tag}-loss.pt')
    # torch.save(ppl, f'result/prompt-ZO-discretized/{tag}-ppl.pt')
...
def main():
    ...
    # model.soft_prompt.data.copy_(
    #     project_soft_prompt_L2_min(
    #         model.soft_prompt.float(), 
    #         model.model.decoder.embed_tokens.weight.float()
    #     )
    # )
    # model.lm_head.weight.data = model.lm_head.weight.data.float()
