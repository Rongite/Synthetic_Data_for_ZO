...
class TrainingArguments(transformers.Seq2SeqTrainingArguments):
    ...
    max_steps: int = field(default=20000, metadata={"help": 'How many optimizer update steps to take'})
    ...
    use_squeezellm: bool = field(default=False)

    squeezellm_ckpt: str = field(default="")
    prefix_tuning: bool = field(default=False)
    lora: bool = field(default=False)

    outlier_percentage: float = field(default=1e-3)
    use_full_zo_update: bool = field(default=False)
...
class GenerationArguments:
    ...
    top_k: Optional[int] = field(default=1)
    diversity_penalty: Optional[float] = field(default=0.0)
    repetition_penalty: Optional[float] = field(default=0.0)
...
def get_accelerate_model(args):
    ...
    dtype = torch.float16
    if args.use_squeezellm:
        from squeezellm_quant import load_quant
        model = load_quant(args.model_name_or_path, 
        ...
        if args.prefix_tuning or args.lora:
            ...
            if args.prefix_tuning:
                ...
                print(f'5 tokens')
                PrefixTuning(model, num_prefix=5, reparam=False, float16=True, init_by_real_act=True)
...
def train():
    ...
    training_args.output_dir += (os.sep + args.model_name_or_path.replace(os.sep, '-'))
    if args.use_squeezellm:
        training_args.output_dir += f"squeezellm-{training_args.outlier_percentage}"

    if args.prefix_tuning:
        training_args.output_dir += "-prefix"
    elif args.lora:
        training_args.output_dir += "-lora"
        
    training_args.output_dir += f"-{args.learning_rate}-{args.zo_eps}-{args.seed}-ZO{os.sep}"
    ...
    from zo_trainer import ZOTrainer
    trainer = ZOTrainer(
        model=model,
        tokenizer=tokenizer,
        args=training_args,
    ...
    if args.do_mmlu_eval:
        ...
        trainer.eval_dataset = mmlu_dataset
