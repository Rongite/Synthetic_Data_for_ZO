...
class SST2Template_LLama2(Template):
    verbalizer = {0: "negative", 1: "positive"}
    def encode(self, sample):
        text = sample.data["sentence"].strip()
        return f"### Sentence: {text} ### Sentiment: "

    def verbalize(self, sample, candidate):
        text = sample.data["sentence"].strip()
        return f"### Sentence: {text} ### Sentiment: {self.verbalizer[candidate]}"
    
    def encode_sfc(self, sample):
        return f" ### Sentiment: "

    def verbalize_sfc(self, sample, candidate):
        return f" ### Sentiment: {self.verbalizer[candidate]}"
...
# class CB_Llama2Template(Template):
#     # From PromptSource 1
#     verbalizer = {0: "Yes", 1: "No", 2: "Maybe"}
...
class RTE_Llama2Template(Template):
    # From PromptSource 1
    verbalizer={0: "Yes", 1: "No"}
    ...
    def encode(self, sample):
        premise = sample.data['premise']
        hypothesis = sample.data['hypothesis']
        return f'Suppose \"{premise}\" Can we infer that \"{hypothesis}\"? Yes or No? '

    def verbalize(self, sample, candidate):
        premise = sample.data['premise']
        hypothesis = sample.data['hypothesis']
        return f'Suppose \"{premise}\" Can we infer that \"{hypothesis}\"? Yes or No? {self.verbalizer[candidate]}'
