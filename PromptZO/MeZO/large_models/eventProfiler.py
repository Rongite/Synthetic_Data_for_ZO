...
class EventProfiler:
    def __init__(self, device, use_offset=True, active=False):
        self.device = device
        # Warm-up GPU
    ...
    def __call__(self, label):
        ...
        if self.use_offset:
            self.cuda_max_mem_data[label].append((torch.cuda.max_memory_allocated() - cuda_mem_offset) / (1024 * 1024)) # MiB
            ...
            self.cuda_max_mem_data[label].append(torch.cuda.max_memory_allocated() / (1024 * 1024))
            self.cuda_allocated_mem_data[label].append(
