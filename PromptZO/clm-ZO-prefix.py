...
def write_results(args, losses, ppl):
    ...
    if args.lora_rank != -1:
        tag += f"-prefix-{args.num_prefix}-{args.lora_rank}-seed-{args.seed}-lr-{args.learning_rate}-eps-{args.ZO_eps}"
    else:
        tag += f"-prefix-{args.num_prefix}-seed-{args.seed}-lr-{args.learning_rate}-eps-{args.ZO_eps}"

    torch.save(losses, f'result/prefix-ZO/{tag}-loss.pt')
    torch.save(ppl, f'result/prefix-ZO/{tag}-ppl.pt')
...
def main():
    ...
    PrefixTuning(model, lora_rank=args.lora_rank, num_prefix=args.num_prefix, reparam=False, init_by_real_act=True, dtype=dtype)
                ...
                if completed_steps > 0 and completed_steps % 100 == 0:
                    ...
                    if args.lora_rank != -1:
                        torch.save({n : p for n, p in model.named_parameters() if p.requires_grad},
                                   f'model/prefix-ZO/{optimizer_name}-{args.dataset_name}-{args.dataset_config_name}-step{completed_steps}_model_{args.block_size}-{args.lora_rank}-{args.learning_rate}-{args.ZO_eps}-{args.seed}.pt')
                    else:
                        torch.save({n: p for n, p in model.named_parameters() if p.requires_grad},
                                   f'model/prefix-ZO/{optimizer_name}-{args.dataset_name}-{args.dataset_config_name}-step{completed_steps}_model_{args.block_size}-{args.learning_rate}-{args.ZO_eps}-{args.seed}.pt')
