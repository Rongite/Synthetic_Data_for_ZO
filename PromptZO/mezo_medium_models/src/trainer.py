...
class Trainer(LinearHeadTrainer):
    ...
    @torch.no_grad()
    def JVP_forward_gradients(self,
                            partial_loss_func, 
        ...
        # Forward AD
        loss, jvp = functorch.jvp(loss_func, (params, ), (v_params,))
    ...
    def train(self, model_path=None, dev_objective=None):
        ...
        if self.args.JVP:
            ...
            def partial_loss_func(input_ids, attention_mask, mask_pos, labels, params):
                ...
                return fmodel(params, buffers, input_ids, attention_
                              mask, None, mask_pos, labels)[0]
        ...
        for epoch in range(epochs_trained, int(num_train_epochs)):
            ...
            # from transformers import RobertaForMaskedLM, RobertaForSequenceClassification, RobertaPreTrainedModel, RobertaModel
            # transformer_seq_cls_model = RobertaForMaskedLM.from_pretrained('roberta-large').cuda()
            # # transformer_seq_cls_model = RobertaModel.from_pretrained('roberta-large').cuda()
            # mezo_model_parameters = model.named_parameters()
            # transformer_model_parameters = transformer_seq_cls_model.named_parameters()

            # for n, p in mezo_model_parameters:
            #     if 'pooler' in n: continue
            #     try:
            #         another_n, another_p = next(transformer_model_parameters)
            #         print(torch.sum(p == another_p))
            #     except:
            #         import pdb; pdb.set_trace()

            # print('\n\n\n')
            # for n, p in transformer_model_parameters:
            #     print(n, ' ', p.shape)
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if self.args.JVP:
                    with torch.no_grad():                    
                        v_params, jvp = self.JVP_forward_gradients(
                            partial_loss_func, 
                            params, 
                            inputs['input_ids'].cuda(), 
                            inputs['attention_mask'].cuda(),
                            inputs['mask_pos'].cuda(),
                            inputs['labels'].cuda()
                        )
                        for v, p in zip(v_params, params):
                            p.sub_(self.args.learning_rate * v * jvp)
