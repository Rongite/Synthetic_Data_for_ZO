...
void vecquant4matmul_spmv_nuq_perchannel_cuda(
  torch::Tensor outlier_rows,
  torch::Tensor outlier_cols,
  torch::Tensor outlier_mat,
  torch::Tensor sensitive_rows,
  torch::Tensor sensitive_cols,
  torch::Tensor sensitive_mat,
  torch::Tensor vec,
  torch::Tensor mul,
  int num_rows,
  torch::Tensor mat4,
  ...
  AT_DISPATCH_FLOATING_TYPES(
    outlier_mat.type(), "spmv_atomic, outlier", ([&] {
      SPMV_ATOMIC<<<num_blocks, block_size>>>(
        outlier_rows.data<int>(),
        outlier_cols.data<int>(),
        outlier_mat.data<scalar_t>(),
        vec.data<scalar_t>(),
        mul.data<scalar_t>(),
        num_rows
      );
  ...
  AT_DISPATCH_FLOATING_TYPES(
    sensitive_mat.type(), "spmv_atomic, sensitive", ([&] {
      SPMV_ATOMIC<<<num_blocks, block_size>>>(
        sensitive_rows.data<int>(),
        sensitive_cols.data<int>(),
        sensitive_mat.data<scalar_t>(),
        vec.data<scalar_t>(),
        mul.data<scalar_t>(),
        num_rows
      );
...
void vecquant3matmul_spmv_nuq_perchannel_batched_cuda(
  ...
  AT_DISPATCH_FLOATING_TYPES(
    outlier_mat.type(), "spmv_atomic_batched, outlier", ([&] {
  ...
  AT_DISPATCH_FLOATING_TYPES(
    sensitive_mat.type(), "spmv_atomic_batched, sensitive", ([&] {
      SPMV_ATOMIC_BATCHED<<<num_blocks, block_size>>>(
        sensitive_rows.data<int>(),
        sensitive_cols.data<int>(),
        sensitive_mat.data<scalar_t>(),
