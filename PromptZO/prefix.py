...
def attn_forward_hook(self, *args, **kwargs):
    ...
    if 'past_key_value' not in kwargs or kwargs['past_key_value'] is None:
        if hasattr(self, 'prefix_keys'):
            prefix_keys, prefix_values = self.prefix_keys, self.prefix_values
        else:
            prefix_keys = self.prefix_keys_1 @ self.prefix_keys_2
            prefix_values = self.prefix_values_1 @ self.prefix_values_2
...
class PrefixTuning:
    ...
    @torch.no_grad()
    def __init__(self, model, num_prefix, lora_rank=-1, reparam=True, embed_dim=512, mid_dim=512, init_by_real_act=False, dtype=torch.float16, extra_r=2048):
        """
        Inputs:
        ...
        for key, _ in model.named_modules():
            if key[-len(attention_name):] == attention_name:
                ...
                self.add_prefix(attn, lora_rank=lora_rank, first=first, input_embeds=input_embeds)

                if init_by_real_act:
                    print(f"Reinitialize with actual activation: {key} (layer {layer_id})")
                    if lora_rank == -1:
                        k_device = attn.prefix_keys.data.device
                        v_device = attn.prefix_values.data.device
                        keys = real_key_values[layer_id][0].squeeze(0).transpose(
                            0, 1).reshape(num_prefix, -1).to(k_device)
                        values = real_key_values[layer_id][1].squeeze(0).transpose(
                            0, 1).reshape(num_prefix, -1).to(v_device)
                        attn.prefix_keys.data = keys
                        attn.prefix_values.data = values
                    else:
                        k_device = attn.prefix_keys_1.device
                        v_device = attn.prefix_values_1.device
                        keys = real_key_values[layer_id][0].squeeze(0).transpose(
                            0, 1).reshape(num_prefix, -1).to(k_device)
                        values = real_key_values[layer_id][1].squeeze(0).transpose(
                            0, 1).reshape(num_prefix, -1).to(v_device)
                        attn.prefix_keys_2.data.copy_(
                            (torch.linalg.pinv(attn.prefix_keys_1.float()) @ keys.float())
                        )
                        attn.prefix_values_2.data.copy_(
                            torch.linalg.pinv(attn.prefix_values_1.float()) @ values.float()
                        )
    ...
    def add_prefix(self, module, lora_rank=-1, first=False, input_embeds=None):
        ...
        if lora_rank == -1:
            ...
            module.prefix_keys_1 = nn.Parameter(torch.randn(self.num_prefix, lora_rank, device=device, dtype=self.model.dtype))
            module.prefix_keys_2 = nn.Parameter(torch.randn(lora_rank, self.hidden_dim, device=device, dtype=self.model.dtype))

            module.prefix_values_1 = nn.Parameter(torch.randn(self.num_prefix, lora_rank, device=device, dtype=self.model.dtype))
            module.prefix_values_2 = nn.Parameter(torch.randn(lora_rank, self.hidden_dim, device=device, dtype=self.model.dtype))
            


class PrefixTuningOneLayer:

    def __init__(self, model, num_prefix, reparam=True, embed_dim=512, mid_dim=512, float16=False, init_by_real_act=False, prefix_layer_id=None):
        """
        Inputs:
        num_prefix: number of prefix tokens
        reparam: use reparameterization trick (not used in MeZO)
    ...
    def add_prefix(self, module, first, input_embeds=None):
        ...
        if self.reparam:
            if first:
                ...
                print("For prefix+reparameterization, inject the embeddings in the first layer.")
